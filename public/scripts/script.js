...
function closeHeaderMenu(e) {
  var timeout = setTimeout(function (e) {
    if (e.relatedTarget === $('.menu-list')[0] || e.relatedTarget === $('.header-search-line')[0]) {
...
$('.header-search-line .button_primary').mouseleave(function (e) {
  closeHeaderMenu(e);
});
$('.menu-list').mouseleave(function (e) {
  if (window.matchMedia('(min-width: 992px)').matches) {
    closeHeaderMenu(e);
  }
});
...
dropzoneForm.on('submit', function (e) {
  ...
  $.ajax({
    ...
    success: function (data) {
      $(this).find('.dropzone-form__files-count').text(data.success === true ? 'Успешная загрузка' : 'Ошибка загрузки файлов');
    }.bind(form),
    error: function () {
      $(this).find('.dropzone-form__files-count').text('Ошибка загрузки файлов');
...
$(document).ready(function () {
  $('.partners').length && $('.partners').marquee({
    duration: 10000,
    duplicated: true,
    gap: 0,
    startVisible: true
...
$(window).scroll(function () {
  var offset = $(window).width() <= 991 ? 140 : 64;
